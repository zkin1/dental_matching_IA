...
global.testUtils = {
  ...
  cleanTestData: async (db) => {
    ...
    try {
      ...
      await db.execute("DELETE FROM asignaciones WHERE id IN (SELECT id FROM asignaciones a INNER JOIN estudiantes_odontologia e ON a.estudiante_id = e.id WHERE e.email LIKE '%test%')");
      ...
      await db.execute("DELETE FROM estudiantes_odontologia WHERE email LIKE '%test%'");
